    ...
    Polinomio operator+(const Polinomio& other) const {
        ...
        while (p != nullptr && q != nullptr) {
            if (p->exponente > q->exponente) {
                ...
                if (suma != 0) {
                    result.push_back(suma, p->exponente);
                }
                p = p->next;
                q = q->next;
            }
...
Polinomio producto1(const Polinomio& P, const Polinomio& Q) {
    Polinomio R;
    for(Node* i=P.head;i!=nullptr; i=i->next){
        for(Node* j=Q.head;j!=nullptr; j=j->next){
            int c = i->coeficiente * j->coeficiente;
            int e = i->exponente + j->exponente;
            R.add(c,e);
        }
    }
    return R;
